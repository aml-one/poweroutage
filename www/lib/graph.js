        ...
        */
        var processListTimeStamp = function(list_timestamp){
            ...
            var start = function(){
                ...
                if (data_date == today)
                {
                    item_html += '<rect class="day today" width="11" height="11" y="'+ y +'" fill="'+ color + '" data-count="'+ count +'" data-date="'+ data_date +'" rx="'+radius+'" ry="'+radius+'"/>';  
                }
                else
                {
            ...
            loop_html +=  '<text text-anchor="middle" class="wday" dx="-10" dy="9">{0}</text>'.formatString( settings.h_days[6])+
                            '<text text-anchor="middle" class="wday" dx="-10" dy="22">{0}</text>'.formatString( settings.h_days[0] )+
                            '<text text-anchor="middle" class="wday" dx="-10" dy="35">{0}</text>'.formatString( settings.h_days[1] )+
                            '<text text-anchor="middle" class="wday" dx="-10" dy="48">{0}</text>'.formatString( settings.h_days[2] )+
                            '<text text-anchor="middle" class="wday" dx="-10" dy="61">{0}</text>'.formatString( settings.h_days[3] )+
                            '<text text-anchor="middle" class="wday" dx="-10" dy="74">{0}</text>'.formatString( settings.h_days[4] )+
                            '<text text-anchor="middle" class="wday" dx="-10" dy="87">{0}</text>'.formatString( settings.h_days[5] );
            ...
            $(_this).find(".day").hover(function () {
                ...
                {
                    $( this ).addClass("today").css("stroke", "black");
                }
                else
